...
export function PromptDetailsView({ keyName }: { keyName: string }) {
  ...
  const onApprove = async (v: number) => {
    try {
      await api.approve('Prompt', keyName, v)
      await load()
  ...
  const onSetActive = async (v: number) => {
    try {
      await api.setActive('Prompt', keyName, v)
      await load()
      setPreviewVersion(null)
  ...
  // Показывать «Сделать активной»:
  // - только в режиме превью
  // - если превью-версия != активной
  // - и она Approved (опционально — если у вас такой статус обязателен)
  const canSetActive =
    isPreviewing &&
    !!displayed &&
    !!active &&
    displayed.version !== active.version &&
    displayed.status === 'Approved' &&
    isAdmin
          ...
          <div className="space-y-3">
            ...
            {canSetActive && (
              <button
                ...
                onClick={() => onSetActive(displayed.version)}
              >
                Сделать активной
              </button>
            )}
      ...
      {editingOpen && (
        <CreatePromptModal
          onSubmit={async (_k, payload) => {
            // Создаём НОВУЮ версию из текущей отображаемой (immutable семантика).
